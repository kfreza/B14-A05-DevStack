...
const TechCard = ({ tech, selected, onAdd }: TechCardProps) => {
  ...
  return (
    <article
      className={`group card flex h-full flex-col justify-between gap-[9.3px] rounded-xl border bg-white p-4.25 font-inter shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-[0_16px_32px_-12px_rgba(219,39,119,0.25)] sm:gap-0 sm:rounded-2xl sm:p-5 sm:font-sans sm:shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] ${
    ...
    >
      <div className="flex flex-col gap-[9.3px] sm:gap-1.5 sm:pb-4">
        <div className="grid grid-cols-[auto_1fr_auto] items-center gap-x-3 sm:grid-cols-[1fr_auto] sm:items-start">
          ...
          {badge && (
            <span
              className={`col-start-3 row-start-1 self-start justify-self-end rounded-full px-2 py-0.5 text-[10.4px] leading-[15.6px] font-bold tracking-[0.208px] sm:col-start-2 sm:border sm:px-[11.4px] sm:py-0.75 sm:text-[11.5px] sm:leading-[17.28px] sm:font-semibold sm:tracking-normal ${badgeTones[badgeTone]}`}
            >
              {badge}
            </span>
          )}
      ...
      <div className="flex flex-col gap-[9.3px] sm:gap-4">
        <div className="flex items-center justify-between border-t border-[#f9fafb] pt-[13.7px] text-[11px] leading-[16.5px] sm:border-slate-50 sm:pt-2.25 sm:font-medium">
          ...
          <span className="flex items-center gap-1 text-[#f59e0b] sm:gap-1.25 sm:font-semibold sm:text-muted">
            ...
            <img src={star} alt="" className="hidden h-2.25 w-[9.285px] sm:block" />
            {rating}
          </span>
        </div>
        <button
